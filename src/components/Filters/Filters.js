...
class Filters extends Component {
  ...
  handleFilterItemPress = (filterValue) => {
    ...
    this.props.filterDrinks({ type: selectedFilterType, value: filterValue.value });
  ...
  isSelected = (filterValue) => {
    const { selectedType, selectedPrice, selectedDistance, selectedFilterType } = this.state;
    ...
    return (
      ...
      >
        <View style={{ flex: 1, justifyContent: 'flex-end', paddingBottom: 30 }}>
          {_.map(filters, filterValue => <FilterItem filterValue={filterValue} handleFilterItemPress={this.handleFilterItemPress} selected={this.isSelected(filterValue)}/>)}
        </View>
      </RBSheet>
    );
  }
  ...
  render() {
    const { filterTypes, selectedType, selectedPrice, selectedDistance } = this.state;
    return (
      <>
        <View style={styles.filterContainer}>
          <View style={styles.stickyItem}>
            <Button disabled style={{ backgroundColor: COLORS.orange, width: 55 }}>
              <Icon
          ...
          >
            {selectedType && selectedType.value !== 'All' && <ActiveFilterButton filterValue={selectedType.text} onPress={() => this.removeFilter('Type')}/>}
            {selectedPrice && selectedPrice.value !== 'All' && <ActiveFilterButton filterValue={selectedPrice.text} onPress={() => this.removeFilter('Price')}/>}
            {selectedDistance && selectedDistance.value !== 'All' && <ActiveFilterButton filterValue={selectedDistance.text} onPress={() => this.removeFilter('Distance')}/>}
            {_.map(filterTypes, filterType => <FilterButton filterType={filterType} handleFilterPress={this.handleFilterPress}/>)}
          </ScrollView>
        </View>
        {this.BottomSheet()}
      </>
